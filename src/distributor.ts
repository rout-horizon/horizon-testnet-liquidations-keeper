...
export class Distributor {
  ...
  constructor(
    private readonly synthetix: Contract,
    private readonly liquidator: Contract,
    ...
    this.logger = createLogger(`Distributor Distributor`);
  ...
  private async indexKeepers(): Promise<number> {
    ...
    while (fromBlock <= latestBlock) {
      ...
      const events = await getEvents(this.getEventsOfInterest(), this.liquidator, {
        fromBlock,
        toBlock,
        logger: this.logger,
      });
